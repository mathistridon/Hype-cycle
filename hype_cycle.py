...
def extract_technologies_data(database_id):
    ...
    for item in data:
        ...
        if selection_hype_cycle:
            technologies.append({'Nom du sujet': tech_name, 'Hype cycle': hype_status, 'Poids dans la phase': phase_weight})
...
def draw_hype_cycle(technologies):
    ...
    vertical_lines = alt.Chart(pd.DataFrame({
        'x': [20, 40, 60, 80]
    })).mark_rule(color='black').encode(
        x='x'
    )

    technologies['x'] = technologies.apply(lambda row: calculate_x_position(row['Hype cycle'], row['Poids dans la phase']), axis=1)
    ...
    for i in range(len(technologies)):
        if i % 2 == 0:
            ...
            technologies.at[i, 'text_y'] = technologies.at[i, 'y'] + (i+1)*2  # Ligne vers le haut
        else:
            technologies.at[i, 'text_x'] = technologies.at[i, 'x']
            technologies.at[i, 'text_y'] = technologies.at[i, 'y'] - (i+1)*2  # Ligne vers le bas

    tech_points = alt.Chart(technologies).mark_point(size=100).encode(
        x='x',
        y='y',
        tooltip=[
    ...
    tech_text = alt.Chart(technologies).mark_text(
        align='left',
        baseline='middle',
        color='red'
    ).encode(
    ...
    connectors = alt.Chart(technologies).mark_line(color='gray').encode(
        x='x',
        x2='text_x',
        y='y',
        y2='text_y'
    ...
    section_text = alt.Chart(section_labels).mark_text(
        baseline='top',
        dy=20,
        fontSize=12
    ).encode(
    ...
    chart = alt.layer(line, vertical_lines, tech_points, connectors, tech_text, section_text)
